...
"""
import numpy as np
from Q_pydy_codegen import eval as func
from dQdq_pydy_codegen import eval as dfdq
from dQdu_pydy_codegen import eval as dfdqd
from dQda_pydy_codegen import eval as dfdqdd
from dQdf_pydy_codegen import eval as dfdf
from scipy.optimize import fsolve
from annimation_cycling import animate_cycling


class PlanarCyclingModelTest(object):
    """
    ...
    def __init__(self, nDoF = 4):

        self.nDoF = nDoF
        
    def _leg_fall(self, ltime= 2):
        
        Nnode = ltime*100
        interval = ltime/Nnode
        time = np.linspace(0, ltime, Nnode)
               
        q = np.zeros((Nnode, self.nDoF))  # [theta_lh(t), theta_lk(t), theta_rh(t), theta_rk(t)]
        qd = np.zeros((Nnode, self.nDoF))  # [omega_lh(t), omega_lk(t), omega_rh(t), omega_rk(t)]
        qdd = np.zeros((Nnode, self.nDoF)) # [beta_lh(t), beta_lk(t), beta_rh(t), beta_rk(t)]
        T = np.zeros((Nnode, self.nDoF))  # [T_lh(t), T_lk(t), T_rh(t), T_rk(t)]
        F = np.zeros((Nnode, self.nDoF))  # [F_lx(t), F_ly(t), F_rx(t), F_ry(t)]
        ...
        animate_cycling(time, q, constants, filename = 'leg_fall_test.mp4')
    ...
    def _derivative_check(self):
        ...
        constants = np.array([1.0, 0.5, 0.4, 0.23, 0.18, 6.3, 4.1, 0.122, 0.041, 9.8])  # [l_St, l_T, l_S, d_T, d_S, m_T, m_S, i_T, i_S, g]
        #res0 = np.zeros(self.nDoF)  #[residuleT_lh, residuleT_lk, residuleF_rh, residuleF_rk]
        jac_dfdq_ini = np.zeros((self.nDoF*self.nDoF))
        jac_dfdqd_ini = np.zeros((self.nDoF*self.nDoF))
        jac_dfdqdd_ini = np.zeros((self.nDoF*self.nDoF))
        jac_dfdf_ini = np.zeros((self.nDoF*self.nDoF))
        ...
        jac_dfdq_pd = np.zeros((self.nDoF, self.nDoF))
        jac_dfdqd_pd = np.zeros((self.nDoF, self.nDoF))
        jac_dfdqdd_pd = np.zeros((self.nDoF, self.nDoF))
        jac_dfdf_pd = np.zeros((self.nDoF, self.nDoF))
        
        res_up = np.zeros(self.nDoF)
        res_dw = np.zeros(self.nDoF)
